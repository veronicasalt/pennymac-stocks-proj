...
IAM: Encountered “Provisioning Missing Trust” errors when deploying the frontend. This was resolved after updating the IAM stack to use a Composite Principal- which allows Amplify and CodeBuild to hold IAM Service Roles.


Weekend and Holiday Gaps: The ingestor logic has to detect not just weekends, but holidays when the market is closed. To account for this, the logic uses a recursive lookback mechanism that checks previous days until it finds valid trading data.
